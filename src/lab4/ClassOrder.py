class Order:
    ...
    def is_valid(self):
        """Проверяем есть ли ошибки в номере телефона и в адресе заказа"""

        phone_split = self.phone.split("-")
        if (
            len(phone_split) < 5
            or self.phone[0] != "+"
            or len(phone_split[0]) != 2
            or len(phone_split[1]) != 3
            or len(phone_split[2]) != 3
            or len(phone_split[3]) != 2
            or len(phone_split[4]) != 2
        ):
            if self.phone == "":
    ...
    def compact_products(self):
        """Создаем словарь где каждому продукту соответствует его количество"""
